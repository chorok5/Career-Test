...
public class JoblistsService {
    public List<JoblistsDTO> getJobs() throws IOException {
        ...
        for (Element jobElement : jobElements) {
            String title = jobElement.select(".cell_mid .cl_top a").text(); 
            String company = jobElement.select(".cell_first .cpname").text();  
            String location = jobElement.select(".cell_mid .cl_md span:nth-of-type(3)").text();  
            String date = jobElement.select(".cell_last .cl_btm span:nth-of-type(1)").text();  
            String detailLink = jobElement.select(".cell_mid .cl_top a").attr("href"); 
